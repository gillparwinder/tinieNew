import { Pressable, StyleSheet, Text, View, Modal, Button, TouchableOpacity } from 'react-native'
...
const SetPasscode = ({ navigation, route }) => {
    const { assistanceData } = route?.params
    ...
    const savedBtn = () => {
        ...
        if (passcode == "") {
            ...
            Toast.show({
                ...
                type: ALERT_TYPE.WARNING,
                title: 'WARNING',
                textBody: 'Confirm Passcode is required',
            })
            setLoading(false)
            ...
            Toast.show({
                ...
                type: ALERT_TYPE.WARNING,
                title: 'WARNING',
                textBody: 'Passcode and confirm Passcode should be same',
            })
            setLoading(false)
        } else {
            setLoading(false)
            ...
            <View style={{ flex: 1, backgroundColor: '#fff' }}>
                ...
                <View style={styles.root}>
                    <View style={styles.textStyle}>
                        <Text style={[styles.text, styles.textFirst]}>Set your 4 digit passcode</Text>
                        <Text style={[styles.text, styles.textSecond]}>(you will use this to login)</Text>
                    </View>
                    <CustomInput placeholder={"ENTER PASSCODE"} value={passcode} onChangeText={(text) => SetPasscode(text)} />
                ...
                >
                    <View style={styles.modelCont}>
                        <View style={styles.modelView}>
                            ...
                            <Pressable
                                onPress={() => btnPressed()} type={'PRIMARY'} ><Text style={styles.modalBtn}>DONE</Text></Pressable>
